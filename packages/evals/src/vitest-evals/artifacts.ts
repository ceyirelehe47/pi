...
export async function persistEvalArtifactReferences(
	...
	for (const artifact of artifacts) {
		if (
			(artifact.type !== "@iris/pi-evals:session" && artifact.type !== "@iris/pi-evals:source") ||
			artifact.runId !== runId
		) {
			continue;
		}
		const category = artifact.type === "@iris/pi-evals:session" ? "sessions" : "sources";
